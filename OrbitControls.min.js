...
console.warn("THREE.OrbitControls: As part of the transition to ES6 Modules, the files in 'examples/js' were deprecated in May 2020 (r117) and will be deleted in December 2020 (r124). You can find more information about developing using ES6 Modules in https://threejs.org/docs/index.html#manual/en/introduction/Import-via-modules."), THREE.OrbitControls = function(e, t) {
		...
		Y = function() {
			...
			return function(t, o) {
				...
				if (r.object.isPerspectiveCamera) {
					...
					var i = e.length();
					i *= Math.tan(r.object.fov / 2 * Math.PI / 270), C(2 * t * i / n.clientHeight, r.object.matrix), S(2 * o * i / n.clientHeight, r.object.matrix)
	...
	function _(e) {
		...
		var n = r.domElement;
		w(2 * Math.PI * R.x / n.clientHeight), j(2 * Math.PI * R.y / n.clientHeight), f.copy(g)
	}
	...
	function F(e) {
		if (!1 !== r.enabled) {
			...
			E !== m.NONE && (document.addEventListener("mousemove", B, !1), document.addEventListener("mouseup", G, !1), r.dispatchEvent(u))
		}
	}

	function B(e) {
		if (!1 !== r.enabled) switch (e.preventDefault(), E) {
			case m.ROTATE:
				if (!1 === r.enableRotate) return;
				! function(e) {
					g.set(e.clientX, e.clientY), R.subVectors(g, f).multiplyScalar(r.rotateSpeed);
					var t = r.domElement;
					w(2 * Math.PI * R.x / t.clientHeight), j(2 * Math.PI * R.y / t.clientHeight), f.copy(g), r.update()
				}(e);
				break;
			case m.DOLLY:
				if (!1 === r.enableZoom) return;
				! function(e) {
					L.set(e.clientX, e.clientY), A.subVectors(L, P), A.y > 0 ? k(N()) : A.y < 0 && D(N()), P.copy(L), r.update()
				}(e);
				break;
			case m.PAN:
				if (!1 === r.enablePan) return;
				! function(e) {
					v.set(e.clientX, e.clientY), y.subVectors(v, H).multiplyScalar(r.panSpeed), Y(y.x, y.y), H.copy(v), r.update()
				}(e)
		}
	}

	function G(e) {
		!1 !== r.enabled && (document.removeEventListener("mousemove", B, !1), document.removeEventListener("mouseup", G, !1), r.dispatchEvent(l), E = m.NONE)
	}

	function K(e) {
		!1 === r.enabled || !1 === r.enableZoom || E !== m.NONE && E !== m.ROTATE || (e.preventDefault(), e.stopPropagation(), r.dispatchEvent(u), function(e) {
	...
	function Q(e) {
		if (!1 !== r.enabled) switch (e.preventDefault(), e.stopPropagation(), E) {
			case m.TOUCH_ROTATE:
				if (!1 === r.enableRotate) return;
				_(e), r.update();
				break;
